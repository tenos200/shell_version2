...
class SimpleShell {
    ...
    commandCase commandChoice(string &formattedUserInput) {
        if (formattedUserInput.compare("exit\n") == 0 ||
            formattedUserInput.compare("exit \n") == 0) {
            return exitCmd;
        } else if (formattedUserInput.compare("history\n") == 0 ||
                   formattedUserInput.compare("history \n") == 0) {
            return historyCmd;
        ...
        if (stringLen == 2 || stringLen == 3) {
            ...
            try {
                ...
                string directoryCmdArgument =
                    formattedUserInput.substr(firstNonCmdCharacter, stringLen);
